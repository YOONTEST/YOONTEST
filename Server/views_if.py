...
from sign.models import Testcase_api
...
def get_testcase(request):
    ...
    if scenario_name != '':
        datas = []
        results = Testcase_api.objects.filter(scenario_name__contains=scenario_name)
        if results:
            for r in results:
                testcase = {}
                testcase['id'] = r.id
    ...
    if project_name != '':
        datas = []
        results = Testcase_api.objects.filter(project_name__contains=project_name)
        if results:
            for r in results:
                testcase = {}
                testcase['id'] = r.id
...
class TestcaseViewSet(viewsets.ModelViewSet):
    ...
    """
    queryset = Testcase_api.objects.all()
